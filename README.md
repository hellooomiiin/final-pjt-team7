...
1. **메인 페이지 (/)**: 인기 영화 카드 표시
   - 홈 버튼, 로그인 버튼, 회원가입 버튼
   - 인기 영화 카드 클릭 시 영화 상세 페이지로 이동

2. **로그인 페이지 (/login)**: 이메일과 비밀번호로 로그인

3. **회원가입 페이지 (/register)**: 이름, 닉네임, 이메일, 비밀번호 입력

4. **영화 상세 페이지 (/movies/:id)**: 영화 상세 정보 표시
   - 영화 포스터, 제목, 줄거리, 평점 등
   - 리뷰 섹션 (상위 3개 미리보기, 최신순 정렬)
   - "더보기 →" 버튼 클릭 시 해당 영화의 리뷰 페이지로 이동

5. **영화별 리뷰 페이지 (/movies/:id/reviews)**: 해당 영화의 모든 리뷰 조회
   - 드롭다운 필터로 리뷰 정렬 (기본값: 최신순)
     - 좋아요 순
     - 높은 평가 순
     - 낮은 평가 순
     - 최신 순
