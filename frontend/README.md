...
│   │   ├── ReviewCreateView.vue  # 리뷰 작성 페이지
│   │   ├── ReviewDetailView.vue  # 리뷰 상세 페이지
│   │   ├── ReviewUpdateView.vue  # 리뷰 수정 페이지
...
- **영화 상세 페이지 (/movies/:id)**: 영화 정보, YouTube 예고편, 리뷰 미리보기
- **영화별 리뷰 페이지 (/movies/:id/reviews)**: 해당 영화의 모든 리뷰 조회 및 정렬
- **리뷰 작성 페이지 (/movies/:id/reviews/create)**: 리뷰 작성 (로그인 필요)
- **리뷰 상세 페이지 (/movies/:id/reviews/:reviewId)**: 리뷰 상세 내용, 좋아요, 댓글
- **리뷰 수정 페이지 (/reviews/:reviewId/update)**: 리뷰 수정 (본인만)
...
- **리뷰 목록 조회**: 영화별 필터링 및 정렬
  - 정렬 필터: 최신순, 오래된순, 좋아요순, 평점 높은순, 평점 낮은순
- **리뷰 좋아요**: 로그인한 사용자만 좋아요 가능, 좋아요 버튼 스타일링
- **댓글 기능**: 리뷰에 댓글 작성 및 삭제 (본인 댓글만 삭제 가능)
- **닉네임 표시**: 리뷰 및 댓글 작성자 닉네임 표시
- **평점 선택**: 라디오 버튼으로 1-5점 선택
- **HTML5 폼 검증**: 제목, 내용 필수 입력 검증
- **중복 리뷰 방지**: 이미 작성한 리뷰가 있으면 알림 표시
...
- ✅ **리뷰 기능 완전 구현**:
  - 리뷰 작성, 수정, 삭제, 상세 조회 페이지 추가
  - 리뷰 목록 페이지 및 정렬 기능
  - 중복 리뷰 작성 방지 기능
  - 로그인 없이도 리뷰 조회 가능
- ✅ **댓글 기능 구현**:
  - 리뷰 상세 페이지에 댓글 작성 및 삭제 기능
  - 로그인한 사용자만 댓글 작성 가능
  - 본인이 작성한 댓글만 삭제 가능
- ✅ **좋아요 기능 구현**:
  - 리뷰 좋아요/좋아요 취소 기능
